...
class HFLLMPredictionMaker:
    ...
    def make_prediction(self, df):
        ...
        recent_data = df.tail(11).reset_index(drop=True)

        # Format the first 10 rows as input
        input_data = recent_data.iloc[:10].to_dict(orient="records")
        input_text = f"""Given the following 10 days of stock data, will the stock price go UP or DOWN on the next day?
...
Respond with only "UP" or "DOWN"."""
        ...
        price_today = recent_data.iloc[9]["Price"]
        price_tomorrow = recent_data.iloc[10]["Price"]
